...
ACTIVE_MUTATION_STATUSES = {"QUEUED", "WORKING"}
...
def evaluate_batch(
    ...
    *,
    ...
    if phase not in {"pending", "quiescent", "terminal", "success"}:
        raise BuildStateError("phase must be pending, quiescent, terminal, or success")
    ...
    for row in builds:
        ...
        if phase == "pending" and status != "PENDING":
            ...
            elif status != "PENDING" and status not in TERMINAL_STATUSES:
                wrong_state.append(trigger)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--phase",
        required=True,
        choices=["pending", "quiescent", "terminal", "success"],
    )
